...
registry_latest() {
  ...
  case "$eco" in
    npm)
      ...
      json=$(vs_retry "$VS_REGISTRY_RETRIES" "$VS_REGISTRY_RETRY_DELAY" curl_with_etag "$VS_ETAG_CACHE_BASE" "$url") || return 1
    ...
    cargo)
      ...
      json=$(vs_retry "$VS_REGISTRY_RETRIES" "$VS_REGISTRY_RETRY_DELAY" curl_with_etag "$VS_ETAG_CACHE_BASE" "$url" -A "version-sentinel (+https://github.com/KSEGIT/Version-Sentinel)") || return 1
